...
def get_existing_ticket_ids(svc):
    result = svc.spreadsheets().values().get(
        spreadsheetId=SPREADSHEET_ID, range=f"{WORKSHEET_NAME}!A2:A",
    ).execute()
    return {str(r[0]) for r in result.get("values", []) if r}

def get_existing_emails(svc):
    """Return lowercase set of customer emails already recorded in the sheet."""
    result = svc.spreadsheets().values().get(
        spreadsheetId=SPREADSHEET_ID, range=f"{WORKSHEET_NAME}!C2:C",
    ).execute()
...
def main():
    ...
    print(f"[Sheets] {len(existing_ids)} existing ticket IDs, {len(existing_emails)} existing customer emails in sheet.")
